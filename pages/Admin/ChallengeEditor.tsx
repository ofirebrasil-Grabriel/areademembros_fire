...
export const ChallengeEditor: React.FC = () => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>, resId: string) => {
    ...
    try {
      ...
      handleResourceChange(resId, 'url', publicUrl);
      
      if (file.type.includes('pdf')) handleResourceChange(resId, 'type', 'pdf');
      else if (file.type.includes('sheet') || file.name.includes('xls')) handleResourceChange(resId, 'type', 'sheet');
      else if (file.type.includes('audio')) handleResourceChange(resId, 'type', 'audio');
      else if (file.type.includes('video')) handleResourceChange(resId, 'type', 'video');
      else if (file.type.includes('image')) handleResourceChange(resId, 'type', 'link');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (day.tasks.length > 0) {
        ...
        await supabase.from('challenge_tasks').upsert(tasksToUpsert);
      }
      ...
      if (day.resources.length > 0) {
        ...
        await supabase.from('challenge_resources').upsert(resToUpsert);
      }
        ...
        <div className="xl:col-span-2 space-y-8">
          <section className="bg-fire-secondary/20 border border-white/5 rounded-xl p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-fire-gray mb-1">Mensagem Matinal</label>
                <textarea 
               ...
               <div>
                <label className="block text-sm font-medium text-fire-gray mb-1">Conceito FIRE</label>
                <textarea 
